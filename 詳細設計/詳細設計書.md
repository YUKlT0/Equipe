...
![](../詳細設計\画面設計UI一覧\カート画面.png)

### 5.2 カテゴリ別商品一覧画面

![](../詳細設計\画面設計UI一覧\カテゴリ別商品一覧画面.png)

### 5.3 マイページ画面

![](../詳細設計/画面設計UI一覧/マイページ画面.png)

### 5.14 ユーザーログイン画面

![](../詳細設計/画面設計UI一覧/ユーザーログイン画面.png)

### 5.5 会員情報編集画面

![](../詳細設計/画面設計UI一覧/会員情報編集画面.png)

### 5.6 管理者ダッシュボード画面

![](../詳細設計/画面設計UI一覧/管理者ダッシュボード画面.png)

### 5.7 管理者ログイン画面

![](../詳細設計/画面設計UI一覧/管理者ログイン画面.png)

### 5.8 管理者編集画面

![](../詳細設計/画面設計UI一覧/管理者編集画面.png)

### 5.9 商品一覧画面

![](../詳細設計/画面設計UI一覧/商品一覧画面.png)

### 5.10 商品詳細画面

![](../詳細設計/画面設計UI一覧/商品詳細画面.png)

### 5.11 商品情報追加・編集画面

![](../詳細設計/画面設計UI一覧/商品情報追加・編集画面.png)

### 5.12 新規会員登録画面

![](../詳細設計/画面設計UI一覧/新規会員登録画面.png)

### 5.13 注文完了画面

![](../詳細設計/画面設計UI一覧/注文完了画面.png)

### 5.14 注文情報確認画面

![](../詳細設計/画面設計UI一覧/注文情報確認画面.png)

### 5.15 注文情報確認画面（管理者）

![](../詳細設計/画面設計UI一覧/注文情報確認画面(管理者).png)

### 5.16 注文情報入力画面

![](../詳細設計/画面設計UI一覧/注文情報入力画面.png)
...
新規会員の登録を行うAPI。ユーザー情報を受け取り、会員として登録する。
...
```json
{
  ...
  "name": "山田 太郎",
  "email": "taro@example.com",
  "registeredAt": "2025-07-08T12:34:56Z"
}
```

#### 失敗レスポンス（HTTP 400 Bad Request）
...
- 入力値をバリデーション（必須、形式、長さなど）
- userIdとemailの一意性を確認
- パスワードはハッシュ化して保存
- 会員情報および住所情報をDBに登録
- registeredAt（登録日時）を付与してレスポンス返却

**備考**

- 登録完了後にログインは行わない（ログインは別API）
- 非会員ユーザーがアクセス可能
- 住所は1対1で会員に紐づける前提（複数住所を扱う場合は別途対応）
...
| Javaバージョン         | OpenJDK 21（Amazon Corretto） |
| フレームワーク         | Spring Boot 3.0               |
| アプリケーションサーバ | Spring Boot 内蔵 Tomcat       |
| Webサーバ              | なし（Spring Boot が直接対応）|
| DB                     | MySQL 8.0                     |
