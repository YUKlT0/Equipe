...
- 各テストケース実行前に、セッションをクリアするか新しいセッションを使用し、データベースもクリーンな状態に初期化する。
- 注文対象の商品データをテスト内容に応じてproductsテーブルに登録する。
  　・正常系では、在庫数や価格が適切に設定された商品（例：商品A、商品B）を用意する。
  　・在庫不足、商品削除などの異常系では、あえて在庫数が足りない商品や存在しない商品IDを用意する。
- セッションには、各シナリオで必要なカート情報を準備する。
  　・1種類または複数商品のカート、在庫ぴったりの数量のカート、在庫不足や商品削除を再現するカート、からのカートなどをテストケースに合わせて設定する。
- リクエストボディには、正常系用の有効な顧客情報、または名前空欄、メール形式不正、不正JSONなどの異常データをテストケースに応じて含める。
- DBの状態変化を確認するため、テスト前後のproductsテーブルの在庫数、ordersテーブルおよびorder_detailsテーブルのレコード数や内容を検証する。
- 注文成功後にセッションのcart属性が削除されていること、またはエラー発生時にカート機能が保持されていることを確認する。
- 模擬的にエラーを発生させるシナリオでは、OderRepository.save、ProductRepository.decreaseStock、CartService.clearCartの各メソッドで例外をスローする設定を行い、ロールバック動作やセッションの状態を検証する。
...
- 手順:
  1. HTTP POSTリクエストを `/api/orders` エンドポイントに送信する。リクエストはOderControllerを経由し、CartServiceでカート情報を取得、OderServiceにて注文処理を実施。
  2. 内部でProductRepositoryによる在庫確認、OderRepositoryによる注文保存、ProductRepository.decreaseStockによる在庫更新、CartService.clearCartによるカート削除が行われる。
- 入力データ:
  - リクエストボディ(JSON):
    ```json
    {
...
- 期待結果:
  ...
  3. DB状態変化:
     - `orders` テーブルに注文情報が正しく登録される。（顧客情報、合計金額1500、ステージ"PENDING"など）。
     - `order_details` テーブルに1件レコードが追加され、商品A(価格500、数量3)が関連付けられる。
...
- 前提条件:
  - DBに商品 `Product(1, "商品A", 500, 10)` が存在する。
  - セッションにカート `Cart(items={"1": CartItem(productId=1, quantity=1, subtotal=500)}, totalQuantity=1, totalPrice=500)` が存在する。
- 手順:
  1. HTTP POSTリクエストを `/api/orders` エンドポイントに送信する。
